...
u8 TP_Init(void)
{
	...
	if(Adujust)
	{
		...
		{//***因触摸屏批次不同等原因，默认的校准参数值可能会引起触摸识别不准，建议校准后再使用，不建议使用固定的默认校准参数
			tp_dev.xfac=0.084038;
			tp_dev.xoff=-11;
			tp_dev.yfac=0.125443;
			tp_dev.yoff=-9;
		}else if(USE_HORIZONTAL==1)
		{//***因触摸屏批次不同等原因，默认的校准参数值可能会引起触摸识别不准，建议校准后再使用，不建议使用固定的默认校准参数
			tp_dev.xfac=-0.085430;
			tp_dev.xoff=333;
			tp_dev.yfac=-0.126006;
			tp_dev.yoff=489;
		}else if(USE_HORIZONTAL==2)
		{//***因触摸屏批次不同等原因，默认的校准参数值可能会引起触摸识别不准，建议校准后再使用，不建议使用固定的默认校准参数
			tp_dev.xfac=0.126483;
			tp_dev.xoff=-12;
			tp_dev.yfac=-0.084393;
			tp_dev.yoff=329;
		}else
		{//***因触摸屏批次不同等原因，默认的校准参数值可能会引起触摸识别不准，建议校准后再使用，不建议使用固定的默认校准参数
			tp_dev.xfac=-0.125087;
			tp_dev.xoff=488;
			tp_dev.yfac=0.084593;
			tp_dev.yoff=-12;
		}
		return 0;
	}else
